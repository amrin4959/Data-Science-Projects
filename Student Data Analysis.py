...
def bubblesort(var1, var2):
    for i in range(len(var2)):
        for j in range(len(var2)-i-1):
            if(var2[j] < var2[j+1]):
                temp = var2[j]
                var2[j] = var2[j+1]
                var2[j+1] = temp
...
print("\n***** Student Information *****")
for name, subjects in student_list.items():
    total=0
    tot_sub = len(subjects.keys())
    for subject, mark in subjects.items():
        total +=mark
        temp_list.append(subject)
        temp_marks.append(mark)
    temp_list,temp_marks = bubblesort(temp_list, temp_marks)
    print(f"\n{name} [Total: {total}, Avg: {total/tot_sub}]\n--------")
    for i in range(3):
        print(f"{temp_list[i]}: {temp_marks[i]}")
...
for subject in subject_list:
    total = 0
    for student, subjects in student_list.items():
        if subject in subjects:
            temp_list.append(student)
            temp_marks.append(subjects[subject])
            total += subjects[subject]
    temp_list, temp_marks = bubblesort(temp_list, temp_marks)
    print(f"\n{subject} [Avg: {total/tot_stu}] \n--------")
    for i in range(3):
        print(f"{temp_list[i]}: {temp_marks[i]}")
...
for name, subjects in student_list.items():
    ...
    for subject, mark in subjects.items():
        if subject in ["Math", "Physics", "Chemistry", "Biology"]:
            totalsci += mark
        elif subject in ["English", "History", "Geography"]:
            totalarts += mark
        elif subject in ["Art", "Music", "PE"]:
            totalextrcurr += mark    
    
    if (totalsci >= 240 and totalsci > totalarts and totalsci > totalextrcurr):
        Science.append(name)
    elif(totalarts >= 160 and totalarts > totalsci and totalarts > totalextrcurr):
        Arts.append(name)
    else:
        Extra_Curricular.append(name)
    
print(f" \nScience Students : {Science} ")
print(f" \nArts Students : {Arts} ")
print(f" \nExtra Curricular Students : {Extra_Curricular} ")    
